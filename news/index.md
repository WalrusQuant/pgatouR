...
- [`pga_player_tournament_status()`](https://walrusquant.github.io/pgatouR/reference/pga_player_tournament_status.md)
  now returns a zero-row tibble when the API has no status to report
  (player not currently in a tournament). It previously returned a
  one-row tibble with all `NA` columns. The column schema is unchanged.
- [`pga_stats()`](https://walrusquant.github.io/pgatouR/reference/pga_stats.md)
  no longer attaches metadata (`stat_title`, `tour_avg`, `year`,
  `display_season`) as attributes. `stat_title`, `stat_id`, and `year`
  are now real columns on the returned tibble; the other metadata was
  dropped (it was silently lost the moment any dplyr verb ran on the
  result anyway).

### New features

- [`pga_stats()`](https://walrusquant.github.io/pgatouR/reference/pga_stats.md)
  accepts vectors for `stat_id` and `year`, looping internally and
  stacking results with `stat_id` and `year` columns.
- [`pga_stats()`](https://walrusquant.github.io/pgatouR/reference/pga_stats.md)
  and
  [`pga_fedex_cup()`](https://walrusquant.github.io/pgatouR/reference/pga_fedex_cup.md)
  gain an `event_query` argument that passes through to the upstream
  `StatDetailEventQuery` variable.
- Three new endpoints fill out documented API coverage:
...
- [`pga_player_tournament_status()`](https://walrusquant.github.io/pgatouR/reference/pga_player_tournament_status.md):
  fixed phantom `roundDisplay` mapping that was always `NA`. The fields
  actually returned by the API (`roundStatusDisplay`,
  `roundStatusColor`) are now surfaced as columns.
- [`pga_player_results()`](https://walrusquant.github.io/pgatouR/reference/pga_player_results.md):
  was hardcoding the first season in the response, silently dropping all
  other seasons. Now returns every season with a `season` column.
- [`pga_player_stats()`](https://walrusquant.github.io/pgatouR/reference/pga_player_stats.md):
  `rank` is now an integer column rather than character.
- [`pga_shot_details()`](https://walrusquant.github.io/pgatouR/reference/pga_shot_details.md):
  no longer crashes with “missing value where TRUE/FALSE needed” when
  the API returns an empty `holes` list (e.g. player with no tracked
  shots that round).
...
- Header-derived column names that previously collided (e.g. two
  `"Rank"` labels in player results, free-form spaces in stat headers)
  are now deduplicated via `make_unique_snake()`.
...
- [`pga_tee_times()`](https://walrusquant.github.io/pgatouR/reference/pga_tee_times.md)
  now builds one tibble per group rather than per player, drastically
  reducing allocations on large fields.
- [`pga_leaderboard()`](https://walrusquant.github.io/pgatouR/reference/pga_leaderboard.md)
  now `%||%`-guards every player/scoring field so a partial response
  from the API can’t break tibble construction. `total_sort` and
  `score_sort` are returned as numeric (they were character).
- [`pga_tournaments()`](https://walrusquant.github.io/pgatouR/reference/pga_tournaments.md)
  ...
  for the `courses` list-column so heterogeneous course schemas no
  longer silently fall back to an empty tibble.
...
- [`pga_scorecard()`](https://walrusquant.github.io/pgatouR/reference/pga_scorecard.md)
  — Hole-by-hole scorecard with par, score, yardage, and status
- [`pga_shot_details()`](https://walrusquant.github.io/pgatouR/reference/pga_shot_details.md)
  — Shot-by-shot tracking data with coordinates and play-by-play
- [`pga_odds()`](https://walrusquant.github.io/pgatouR/reference/pga_odds.md)
  — Betting odds to win for the tournament field
- [`pga_coverage()`](https://walrusquant.github.io/pgatouR/reference/pga_coverage.md)
  — Broadcast and streaming schedule

#### Statistics & Standings

- [`pga_stats()`](https://walrusquant.github.io/pgatouR/reference/pga_stats.md)
  — Any of 300+ stats with full player rankings (2004-2026)
- [`pga_fedex_cup()`](https://walrusquant.github.io/pgatouR/reference/pga_fedex_cup.md)
  — FedExCup standings with projected and official rankings
- [`pga_scorecard_comparison()`](https://walrusquant.github.io/pgatouR/reference/pga_scorecard_comparison.md)
  — Head-to-head stat comparison between players

#### Players & Tournaments

- [`pga_players()`](https://walrusquant.github.io/pgatouR/reference/pga_players.md)
  — Full player directory (2,400+ players across PGA Tour, Champions,
  Korn Ferry)
- [`pga_tournaments()`](https://walrusquant.github.io/pgatouR/reference/pga_tournaments.md)
...
- [`pga_news()`](https://walrusquant.github.io/pgatouR/reference/pga_news.md)
  — News articles with filtering and pagination
- [`pga_news_franchises()`](https://walrusquant.github.io/pgatouR/reference/pga_news_franchises.md)
...
- `stat_ids` — Bundled lookup table of 340 stat IDs with names,
