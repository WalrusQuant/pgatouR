...
An R client for the PGA Tour API. Access leaderboards, player stats,
scorecards, shot tracking data, tee times, FedExCup standings, betting
odds, broadcast schedules, news, and video highlights — all returned as
tidy tibbles.
...
e.g., `"R2026475"` for the 2026 Valspar Championship on the PGA Tour.

You can find tournament IDs from the PGA Tour website URL or by checking
the leaderboard/schedule pages. The
[`pga_tournaments()`](https://walrusquant.github.io/pgatouR/reference/pga_tournaments.md)
function can retrieve metadata for known IDs.
...
- **Authentication:** Uses a public API key embedded in the PGA Tour
  frontend (no user authentication required)
