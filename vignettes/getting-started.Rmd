...
vignette: >
  %\VignetteIndexEntry{Getting Started with pgatouR}
...
You can find these from PGA Tour URLs or by using `pga_schedule()` to list all tournaments for a season:

```{r schedule-ids}
# Full 2025 season (50 events with IDs and names)
pga_schedule(2025)
#> # A tibble: 50 × 2
#>    tournament_id tournament_name
#>    <chr>         <chr>
#>  1 088           PGA TOUR Q-School presented by Korn Ferry
#>  2 551           Grant Thornton Invitational
#>  3 478           Hero World Challenge
#>  ...

# Current season (shows completed events so far)
pga_schedule(2026)
```
...
The `courses` column is a list-column containing a tibble of course details for each tournament.
...
Note: `pga_videos()` takes the numeric tournament ID (`"475"`) without the tour code prefix, while most other functions use the full ID (`"R2026475"`).
...
The package includes a bundled `stat_ids` dataset with all 340 known stat IDs:
...
Get tournament IDs and names for any season. Past seasons return the full schedule; the current season returns events played so far.

```{r schedule}
# Full 2025 season
schedule <- pga_schedule(2025)
schedule
#> # A tibble: 50 × 2
#>    tournament_id tournament_name
#>    <chr>         <chr>
...
# Use the IDs with other functions
# (prepend tour code + year, e.g., "R2025016" for The Sentry)
pga_leaderboard("R2025016")
```
...
- **`stat_ids`** is lazy-loaded — just type `stat_ids` to access it, no `data()` call needed.
